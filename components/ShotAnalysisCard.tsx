...
export default function ShotAnalysisCard({ title, subtitle, lastScore, sparklineScores, onClick }: Props) {
  ...
  const h = 56;
  const pad = 6;
  const { line, area } = buildSparkPath(scores.slice().reverse(), w, h, pad);
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {subtitle ? <div className="mt-0.5 text-[11px] text-white/60">{subtitle}</div> : null}
        </div>
        ...
        <div className="shrink-0 text-right">
          ...
          {typeof lastScore === "number" ? (
            <div className="mt-1 text-[11px] text-white/60">Latest score</div>
          ) : (
            <div className="mt-1 text-[11px] text-white/60">Not analyzed yet</div>
          )}
        </div>
      </div>
      ...
      <div className="mt-3">
        {scores.length > 0 ? (
          ...
          >
            ...
            <path d={line} fill="none" stroke="currentColor" className="text-white/70" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
          </svg>
        ) : (
